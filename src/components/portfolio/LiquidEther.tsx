 ...
 *  - Typed, and the 5-line stylesheet is folded into the root className so the
 *    project keeps its "Tailwind only, no component CSS" convention.
 *  - Re-initialisation is keyed on the palette alone. Upstream listed every
 *    prop in the effect deps, so nudging `mouseForce` tore down and rebuilt the
 *    WebGL context; here the simulation is built once and live props are pushed
...
export type LiquidEtherProps = {
  /** Hex colour stops, low velocity → high velocity. */
  colors?: string[];
  mouseForce?: number;
  cursorSize?: number;
  isViscous?: boolean;
   ...
   */
  dissipation?: number;
  /** Simulation scale relative to canvas size. Lower = softer and cheaper. */
  resolution?: number;
  isBounce?: boolean;
  autoDemo?: boolean;
  autoSpeed?: number;
...
export default function LiquidEther({
  ...
  // Live prop mirror: the simulation is built once, then these are pushed onto
  // the running instance, so prop tweaks never rebuild the WebGL context.
  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      // Wall-clock timing only; the simulation advances on a fixed `dt`.
      // (Upstream used THREE.Clock, which three has since deprecated.)
      lastTime = 0;
      time = 0;
      delta = 0;
    ...
    class MouseClass {
      ...
      private _onMouseMove = (e: MouseEvent) => this.onDocumentMouseMove(e);
      private _onTouchStart = (e: TouchEvent) => this.onDocumentTouchStart(e);
      private _onTouchMove = (e: TouchEvent) => this.onDocumentTouchMove(e);
...
`;
    ...
    // Alpha alone carries intensity; RGB stays at full chroma. Upstream also
    // scaled RGB by lenv, which reads as a glow against the demo's black
    // backdrop but as a translucent grey smudge on a light canvas.
    float outA = mix(bgColor.a, 1.0, lenv);
    gl_FragColor = vec4(mix(bgColor.rgb, c, step(0.0001, lenv)), outA);
}
`;
...
`;
    ...
    type PassProps = {
      material?: { vertexShader: string; fragmentShader: string; uniforms: Uniforms };
    ...
    class Simulation {
      ...
      dispose() {
        for (const key in this.fbos) this.fbos[key].dispose();
      }
      update() {
    ...
    class WebGLManager {
      ...
      constructor(props: {
        ...
        this.output = new Output(props.sim);
    ...
    try {
      webgl = new WebGLManager({
        ...
        autoIntensity: p.autoIntensity,
        takeoverDuration: p.takeoverDuration,
        autoResumeDelay: p.autoResumeDelay,
        autoRampDuration: p.autoRampDuration,
        sim: {
    ...
    // Pause when scrolled out of view so the sim costs nothing off-screen.
  ...
  // Push live prop changes onto the running simulation.
  useEffect(() => {
  ...
  return (
    <div
      ref={mountRef}
      className={`relative h-full w-full overflow-hidden [touch-action:none] ${className}`}
      style={style}
    />
  );
}
